...
#                      '/mnt/nvme1/models_decloud/','/mnt/nvme1/models_decloud/', 150, 8,2)
...
if __name__ == '__main__':
    ...
    model = Sentinel12UNetTopo().to(device)
    date = datetime.now().strftime('%Y-%m-%d')
    project_name = f'Sentinel12UNetTopo_Full_No_EarlyStop_Batch_8_150_{date}'
...
# Create the SummaryWriter with the specified log directory and filename_suffix
    ...
    s2_10 = torch.randn([1, 4, 256, 256]).to(device)
    s2_20 = torch.randn([1, 6, 256,256]).to(device)
    s1_10 = torch.randn([1, 2,256, 256]).to(device)
    aspect_20 = torch.randn([1, 1, 256, 256]).to(device)
    slope_20 = torch.randn([1, 1, 256,256]).to(device)
    hillshade_20 = torch.randn([1, 1,256, 256]).to(device)
    dem_20 = torch.randn([1, 1, 128, 128]).to(device)
    writer.add_graph(model, input_to_model=(s2_10, s2_20, s1_10, aspect_20, slope_20, hillshade_20, dem_20))
            ...
            if 1 == 1 :
                ...
                if epoch % 1 == 0:
                    ...
                    for idx in range(num_figures):
                        img_1 = np.rollaxis(np.squeeze(5*s2_10_input[idx][[2, 1, 0], :, :].cpu().numpy()), 0, 3)
